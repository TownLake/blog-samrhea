...
const HealthDashboard = () => {
  ...
  // Dynamic import (no Suspense) for WeightDeficitModal
  const [LazyDeficitModal, setLazyDeficitModal] = useState(null);
  ...
  useEffect(() => {
    ...
    if (hash === 'cumulative_deficit') {
      ...
      // preload the modal when opening
      if (!LazyDeficitModal) {
        import('/src/features/health/components/WeightDeficitModal.jsx')
          .then(m => setLazyDeficitModal(() => m.default))
          .catch((e) => console.error('Failed to load WeightDeficitModal:', e));
      }
    } else {
  ...
  return (
    <div className="pt-2 pb-8">
      ...
      {isDeficitModalOpen && LazyDeficitModal && (
        <LazyDeficitModal
