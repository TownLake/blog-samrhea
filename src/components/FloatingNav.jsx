...
import { NAVIGATION_MAP, PRIMARY_NAV_SECTIONS } from '../constants';
  ...
  return (
    <div ref={navRef} className="fixed bottom-0 left-1/2 -translate-x-1/2 z-50 flex justify-center mb-5 px-4">
      <div className="relative">
        {/* Expanded Vertical Menu - Positioned relative to the nav bar */}
        <div 
          ...
          <div className="flex items-center gap-1">
            {currentPrimarySection.subnav.map((option) => (
              <NavLink
                key={option.id}
                to={option.path}
                end // Ensures the NavLink is only active on an exact match
