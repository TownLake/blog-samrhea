...
const METRIC_RANGES = {
  ...
  // Note: 'five_k_seconds' does not have categories and is handled separately.
};
...
export const getMetricCategory = (metricKey, value) => {
  ...
  if (metricKey === 'five_k_seconds' || metricKey === 'weight') { // Added 'weight' as it often doesn't have these categories
    return 'default';
  }
...
export const getMetricCategoryInfo = (metricKey, value) => {
  ...
  if (metricKey === 'five_k_seconds' || metricKey === 'weight' || category === 'default') {
    return {
      category: 'default',
